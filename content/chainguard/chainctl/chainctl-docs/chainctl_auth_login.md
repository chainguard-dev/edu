...
```
chainctl auth login [--invite-code=INVITE_CODE | --register] [--identity-token=PATH_TO_TOKEN] [--identity=IDENTITY_ID] [--identity-provider=IDP_ID] [--org-name=ORG_NAME] [--social-login={google|github|gitlab}] [--cluster=CLUSTER] [--headless] [--prefer-ambient-credentials] [--refresh] [--output table|id|json|none]
```

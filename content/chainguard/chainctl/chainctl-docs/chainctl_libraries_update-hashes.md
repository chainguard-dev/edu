...
Authentication:
  By default, chainctl pulls a Chainguard token via 'chainctl auth pull-token'. To
  authenticate without an issuer connection (e.g. from a rebuilder workflow), pass
  --token or set CHAINCTL_AUTH_TOKEN. To authenticate against a private proxy
  (e.g. Artifactory/JFrog) with real basic-auth credentials, use --username and
...
Custom registry URLs:
  ...
  consulted (CHAINCTL_AUTH_TOKEN env var, ~/.chainguard/token file, and
  'chainctl auth pull-token') — sending the Chainguard JWT as a basic-auth

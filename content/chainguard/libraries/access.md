...
Prior to `chainctl` version 0.2.290, cooldown policies were enabled via the `chainctl entitlements command`. Cooldown policies from earlier versions are listed as read-only system policies. To change the cooldown period, create a new custom policy and enforce it for a specified ecosystem.

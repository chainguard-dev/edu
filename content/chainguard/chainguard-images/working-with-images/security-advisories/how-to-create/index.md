...
aliases: 
type: "article"
description: "Guide on how to use the wolfictl tool to view, create, and update security advisories"
...
First, we will be asked to enter the package in which the CVE was found. For our demonstration we will use the `pkgnamehere` package. Then, we will be asked for the vulnerability ID that we wish to make an advisory for. This ID must be a CVE, CGA, GHSA, or Go vulnerability ID. Finally, we will be asked what the status for the advisory should be, from one of the following options:
detection (Under investigation)
true-positive-determination (Affected)
fixed (Fixed)
false-positive-determination (Not affected)
analysis-not-planned (what does this correspond to? couldn’t find a single entry with it)
fix-not-planned (Fix not planned)
pending-upstream-fix (Pending upstream fix)
To learn more about the meanings of each of these identifiers, please refer to our [document outlining how security advisories are created](/content/chainguard/chainguard-images/working-with-images/security-advisories/how-chainguard-issues/). The following shows an example of the `wolfictl advisory create` command in action.

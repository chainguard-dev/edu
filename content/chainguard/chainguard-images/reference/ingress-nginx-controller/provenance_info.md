...
### Registry and Tags for ingress-nginx-controller Image
Attestations are provided per image build, so you'll need to specify the correct tag and registry when pulling attestations from an image with `cosign`.

| Registry                     | Tags                                                                                                                                                                                                                                           |
|------------------------------|------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| `cgr.dev/chainguard`         | latest, latest-dev                                                                                                                                                                                                                             |
| `cgr.dev/chainguard-private` | 1, 1-dev, 1.10, 1.10-dev, 1.10.0, 1.10.0-dev, 1.10.1, 1.10.1-dev, 1.8, 1.8-dev, 1.8.2, 1.8.2-dev, 1.9, 1.9-dev, 1.9.0, 1.9.0-dev, 1.9.1, 1.9.1-dev, 1.9.3, 1.9.3-dev, 1.9.4, 1.9.4-dev, 1.9.5, 1.9.5-dev, 1.9.6, 1.9.6-dev, latest, latest-dev |
...
## Verifying ingress-nginx-controller Image Signatures
...
cosign verify \
  ...
  cgr.dev/chainguard/ingress-nginx-controller | jq
```
...
cgr.dev/chainguard-private/ingress-nginx-controller | jq
```

## Downloading ingress-nginx-controller Image Attestations

The following [attestations](https://slsa.dev/attestation-model) for the ingress-nginx-controller image can be obtained and verified via cosign:
...
To download an attestation, use the `cosign download attestation` command and provide both the predicate type and the build platform. For example, the following command will obtain the SBOM for the ingress-nginx-controller image on `linux/amd64`:
...
cosign download attestation \
  ...
  cgr.dev/chainguard/ingress-nginx-controller | jq -r .payload | base64 -d | jq .predicate
```
...
--platform=linux/amd64 \
--predicate-type=https://spdx.dev/Document \
cgr.dev/chainguard-private/ingress-nginx-controller | jq -r .payload | base64 -d | jq .predicate
```
...
## Verifying ingress-nginx-controller Image Attestations
You can use the `cosign verify-attestation` command to check the signatures of the ingress-nginx-controller image attestations:
...
--certificate-oidc-issuer=https://token.actions.githubusercontent.com \
--certificate-identity=https://github.com/chainguard-images/images-private/.github/workflows/release.yaml@refs/heads/main \
cgr.dev/chainguard-private/ingress-nginx-controller
```

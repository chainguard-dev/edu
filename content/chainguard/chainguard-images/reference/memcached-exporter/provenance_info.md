...
### Registry and Tags for memcached-exporter Image
Attestations are provided per image build, so you'll need to specify the correct tag and registry when pulling attestations from an image with `cosign`.

| Registry                     | Tags                                                                                                             |
|------------------------------|------------------------------------------------------------------------------------------------------------------|
| `cgr.dev/chainguard`         | latest                                                                                                           |
| `cgr.dev/chainguard-private` | 0, 0-dev, 0.11, 0.11.3, 0.13, 0.13.0, 0.14, 0.14-dev, 0.14.2, 0.14.2-dev, 0.14.3, 0.14.3-dev, latest, latest-dev |
...
cosign verify \
  ...
  cgr.dev/chainguard/memcached-exporter | jq
```
...
cgr.dev/chainguard-private/memcached-exporter | jq
```
...
To download an attestation, use the `cosign download attestation` command and provide both the predicate type and the build platform. For example, the following command will obtain the SBOM for the memcached-exporter image on `linux/amd64`:
...
--platform=linux/amd64 \
--predicate-type=https://spdx.dev/Document \
cgr.dev/chainguard-private/memcached-exporter | jq -r .payload | base64 -d | jq .predicate
```
...
--certificate-oidc-issuer=https://token.actions.githubusercontent.com \
--certificate-identity=https://github.com/chainguard-images/images-private/.github/workflows/release.yaml@refs/heads/main \
cgr.dev/chainguard-private/memcached-exporter
```

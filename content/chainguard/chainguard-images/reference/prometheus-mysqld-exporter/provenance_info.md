...
{{< tab title="Provenance" active=true url="/chainguard/chainguard-images/reference/prometheus-mysqld-exporter/provenance_info/" >}}
{{</ tabs >}}
...
### Registry and Tags for prometheus-mysqld-exporter Image
Attestations are provided per image build, so you'll need to specify the correct tag and registry when pulling attestations from an image with `cosign`.

| Registry                     | Tags                                                                                               |
|------------------------------|----------------------------------------------------------------------------------------------------|
| `cgr.dev/chainguard`         | 0, 0-dev, 0.15, 0.15-dev, 0.15.1, 0.15.1-dev, latest, latest-dev                                   |
| `cgr.dev/chainguard-private` | 0, 0-dev, 0.14, 0.14.0, 0.15, 0.15-dev, 0.15.0, 0.15.0-dev, 0.15.1, 0.15.1-dev, latest, latest-dev |
...
## Verifying prometheus-mysqld-exporter Image Signatures
...
cosign verify \
  ...
  cgr.dev/chainguard/prometheus-mysqld-exporter | jq
```
...
cgr.dev/chainguard-private/prometheus-mysqld-exporter | jq
```

## Downloading prometheus-mysqld-exporter Image Attestations

The following [attestations](https://slsa.dev/attestation-model) for the prometheus-mysqld-exporter image can be obtained and verified via cosign:
...
To download an attestation, use the `cosign download attestation` command and provide both the predicate type and the build platform. For example, the following command will obtain the SBOM for the prometheus-mysqld-exporter image on `linux/amd64`:
...
cosign download attestation \
  ...
  cgr.dev/chainguard/prometheus-mysqld-exporter | jq -r .payload | base64 -d | jq .predicate
```
...
--platform=linux/amd64 \
--predicate-type=https://spdx.dev/Document \
cgr.dev/chainguard-private/prometheus-mysqld-exporter | jq -r .payload | base64 -d | jq .predicate
```
...
## Verifying prometheus-mysqld-exporter Image Attestations
You can use the `cosign verify-attestation` command to check the signatures of the prometheus-mysqld-exporter image attestations:
...
--certificate-oidc-issuer=https://token.actions.githubusercontent.com \
--certificate-identity=https://github.com/chainguard-images/images-private/.github/workflows/release.yaml@refs/heads/main \
cgr.dev/chainguard-private/prometheus-mysqld-exporter
```

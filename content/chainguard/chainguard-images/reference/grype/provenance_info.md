...
### Registry and Tags for grype Image
Attestations are provided per image build, so you'll need to specify the correct tag and registry when pulling attestations from an image with `cosign`.

| Registry                     | Tags                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
|------------------------------|------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| `cgr.dev/chainguard`         | latest, latest-dev                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
| `cgr.dev/chainguard-private` | 0, 0-dev, 0.73, 0.73-dev, 0.73.1, 0.73.1-dev, 0.73.2, 0.73.2-dev, 0.73.3, 0.73.3-dev, 0.73.4, 0.73.4-dev, 0.73.5, 0.73.5-dev, 0.74, 0.74-dev, 0.74.0, 0.74.0-dev, 0.74.1, 0.74.1-dev, 0.74.2, 0.74.2-dev, 0.74.3, 0.74.3-dev, 0.74.4, 0.74.4-dev, 0.74.5, 0.74.5-dev, 0.74.6, 0.74.6-dev, 0.74.7, 0.74.7-dev, 0.75, 0.75-dev, 0.75.0, 0.75.0-dev, 0.76, 0.76-dev, 0.76.0, 0.76.0-dev, 0.77, 0.77-dev, 0.77.0, 0.77.0-dev, 0.77.1, 0.77.1-dev, 0.77.2, 0.77.2-dev, 0.77.3, 0.77.3-dev, 0.77.4, 0.77.4-dev, 0.78, 0.78-dev, 0.78.0, 0.78.0-dev, latest, latest-dev |
...
## Verifying grype Image Signatures
...
cgr.dev/chainguard-private/grype | jq
```

## Downloading grype Image Attestations

The following [attestations](https://slsa.dev/attestation-model) for the grype image can be obtained and verified via cosign:
...
To download an attestation, use the `cosign download attestation` command and provide both the predicate type and the build platform. For example, the following command will obtain the SBOM for the grype image on `linux/amd64`:
...
cosign download attestation \
  ...
  cgr.dev/chainguard/grype | jq -r .payload | base64 -d | jq .predicate
```
...
--platform=linux/amd64 \
--predicate-type=https://spdx.dev/Document \
cgr.dev/chainguard-private/grype | jq -r .payload | base64 -d | jq .predicate
```
...
## Verifying grype Image Attestations
You can use the `cosign verify-attestation` command to check the signatures of the grype image attestations:
...
--certificate-oidc-issuer=https://token.actions.githubusercontent.com \
--certificate-identity=https://github.com/chainguard-images/images-private/.github/workflows/release.yaml@refs/heads/main \
cgr.dev/chainguard-private/grype
```

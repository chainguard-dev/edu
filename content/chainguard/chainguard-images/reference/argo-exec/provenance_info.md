...
### Registry and Tags for argo-exec Image
...
## Verifying argo-exec Image Signatures
...
cgr.dev/chainguard-private/argo-exec | jq
```

## Downloading argo-exec Image Attestations

The following [attestations](https://slsa.dev/attestation-model) for the argo-exec image can be obtained and verified via cosign:
...
cosign download attestation \
  ...
  cgr.dev/chainguard/argo-exec | jq -r .payload | base64 -d | jq .predicate
```
...
--platform=linux/amd64 \
--predicate-type=https://spdx.dev/Document \
cgr.dev/chainguard-private/argo-exec | jq -r .payload | base64 -d | jq .predicate
```
...
## Verifying argo-exec Image Attestations
You can use the `cosign verify-attestation` command to check the signatures of the argo-exec image attestations:
...
--certificate-oidc-issuer=https://token.actions.githubusercontent.com \
--certificate-identity=https://github.com/chainguard-images/images-private/.github/workflows/release.yaml@refs/heads/main \
cgr.dev/chainguard-private/argo-exec
```

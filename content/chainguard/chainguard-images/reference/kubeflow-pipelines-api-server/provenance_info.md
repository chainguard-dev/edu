...
### Registry and Tags for kubeflow-pipelines-api-server Image
...
## Verifying kubeflow-pipelines-api-server Image Signatures
...
cosign verify \
  ...
  cgr.dev/chainguard/kubeflow-pipelines-api-server | jq
```
...
cgr.dev/chainguard-private/kubeflow-pipelines-api-server | jq
```

## Downloading kubeflow-pipelines-api-server Image Attestations

The following [attestations](https://slsa.dev/attestation-model) for the kubeflow-pipelines-api-server image can be obtained and verified via cosign:
...
To download an attestation, use the `cosign download attestation` command and provide both the predicate type and the build platform. For example, the following command will obtain the SBOM for the kubeflow-pipelines-api-server image on `linux/amd64`:
...
cosign download attestation \
  ...
  cgr.dev/chainguard/kubeflow-pipelines-api-server | jq -r .payload | base64 -d | jq .predicate
```
...
--platform=linux/amd64 \
--predicate-type=https://spdx.dev/Document \
cgr.dev/chainguard-private/kubeflow-pipelines-api-server | jq -r .payload | base64 -d | jq .predicate
```
...
## Verifying kubeflow-pipelines-api-server Image Attestations
You can use the `cosign verify-attestation` command to check the signatures of the kubeflow-pipelines-api-server image attestations:
...
--certificate-oidc-issuer=https://token.actions.githubusercontent.com \
--certificate-identity=https://github.com/chainguard-images/images-private/.github/workflows/release.yaml@refs/heads/main \
cgr.dev/chainguard-private/kubeflow-pipelines-api-server
```

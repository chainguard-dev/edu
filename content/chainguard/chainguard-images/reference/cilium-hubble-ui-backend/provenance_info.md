...
### Registry and Tags for cilium-hubble-ui-backend Image
...
## Verifying cilium-hubble-ui-backend Image Signatures
...
cosign verify \
  ...
  cgr.dev/chainguard/cilium-hubble-ui-backend | jq
```
...
cgr.dev/chainguard-private/cilium-hubble-ui-backend | jq
```

## Downloading cilium-hubble-ui-backend Image Attestations

The following [attestations](https://slsa.dev/attestation-model) for the cilium-hubble-ui-backend image can be obtained and verified via cosign:
...
To download an attestation, use the `cosign download attestation` command and provide both the predicate type and the build platform. For example, the following command will obtain the SBOM for the cilium-hubble-ui-backend image on `linux/amd64`:
...
cosign download attestation \
  ...
  cgr.dev/chainguard/cilium-hubble-ui-backend | jq -r .payload | base64 -d | jq .predicate
```
...
--platform=linux/amd64 \
--predicate-type=https://spdx.dev/Document \
cgr.dev/chainguard-private/cilium-hubble-ui-backend | jq -r .payload | base64 -d | jq .predicate
```
...
## Verifying cilium-hubble-ui-backend Image Attestations
You can use the `cosign verify-attestation` command to check the signatures of the cilium-hubble-ui-backend image attestations:
...
--certificate-oidc-issuer=https://token.actions.githubusercontent.com \
--certificate-identity=https://github.com/chainguard-images/images-private/.github/workflows/release.yaml@refs/heads/main \
cgr.dev/chainguard-private/cilium-hubble-ui-backend
```

---
title: "Provenance Information for graalvm-native Images"
type: "article"
unlisted: true
description: "Provenance information for graalvm-native Chainguard Image"
...
{{< tab title="Provenance" active=true url="/chainguard/chainguard-images/reference/graalvm-native/provenance_info/" >}}
{{</ tabs >}}
...
### Registry and Tags for graalvm-native Image
...
## Verifying graalvm-native Image Signatures
The **graalvm-native** Chainguard Images are signed using Sigstore, and you can check the included signatures using `cosign`.
...
cosign verify \
  ...
  cgr.dev/chainguard/graalvm-native | jq
```
...
cgr.dev/chainguard-private/graalvm-native | jq
```

## Downloading graalvm-native Image Attestations

The following [attestations](https://slsa.dev/attestation-model) for the graalvm-native image can be obtained and verified via cosign:
...
To download an attestation, use the `cosign download attestation` command and provide both the predicate type and the build platform. For example, the following command will obtain the SBOM for the graalvm-native image on `linux/amd64`:
...
cosign download attestation \
  ...
  cgr.dev/chainguard/graalvm-native | jq -r .payload | base64 -d | jq .predicate
```
...
--platform=linux/amd64 \
--predicate-type=https://spdx.dev/Document \
cgr.dev/chainguard-private/graalvm-native | jq -r .payload | base64 -d | jq .predicate
```
...
## Verifying graalvm-native Image Attestations
You can use the `cosign verify-attestation` command to check the signatures of the graalvm-native image attestations:
...
cosign verify-attestation \
  ...
  --certificate-oidc-issuer=https://token.actions.githubusercontent.com \
  --certificate-identity=https://github.com/chainguard-images/images/.github/workflows/release.yaml@refs/heads/main \
  cgr.dev/chainguard/graalvm-native
```
...
--certificate-oidc-issuer=https://token.actions.githubusercontent.com \
--certificate-identity=https://github.com/chainguard-images/images-private/.github/workflows/release.yaml@refs/heads/main \
cgr.dev/chainguard-private/graalvm-native
```
...
```
Verification for cgr.dev/chainguard/graalvm-native --

...
{{< tab title="Details" active=false url="/chainguard/chainguard-images/reference/notification-controller/image_specs/" >}}
...
### Registry and Tags for notification-controller Image
...
## Verifying notification-controller Image Signatures
...
cosign verify \
  ...
  cgr.dev/chainguard/notification-controller | jq
```
...
cgr.dev/chainguard-private/notification-controller | jq
```

## Downloading notification-controller Image Attestations

The following [attestations](https://slsa.dev/attestation-model) for the notification-controller image can be obtained and verified via cosign:
...
To download an attestation, use the `cosign download attestation` command and provide both the predicate type and the build platform. For example, the following command will obtain the SBOM for the notification-controller image on `linux/amd64`:
...
cosign download attestation \
  ...
  cgr.dev/chainguard/notification-controller | jq -r .payload | base64 -d | jq .predicate
```
...
--platform=linux/amd64 \
--predicate-type=https://spdx.dev/Document \
cgr.dev/chainguard-private/notification-controller | jq -r .payload | base64 -d | jq .predicate
```
...
## Verifying notification-controller Image Attestations
You can use the `cosign verify-attestation` command to check the signatures of the notification-controller image attestations:
...
--certificate-oidc-issuer=https://token.actions.githubusercontent.com \
--certificate-identity=https://github.com/chainguard-images/images-private/.github/workflows/release.yaml@refs/heads/main \
cgr.dev/chainguard-private/notification-controller
```

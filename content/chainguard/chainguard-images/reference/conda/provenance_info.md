...
### Registry and Tags for conda Image
Attestations are provided per image build, so you'll need to specify the correct tag and registry when pulling attestations from an image with `cosign`.

| Registry                     | Tags                                                                                                                                                                                                                                                                                                                                               |
|------------------------------|----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| `cgr.dev/chainguard`         | latest, latest-dev                                                                                                                                                                                                                                                                                                                                 |
| `cgr.dev/chainguard-private` | 23, 23-dev, 23.5, 23.5-dev, 23.5.0, 23.5.0-dev, 23.5.2, 23.5.2-dev, 23.7, 23.7-dev, 23.7.0, 23.7.0-dev, 23.7.2, 23.7.2-dev, 23.7.3, 23.7.3-dev, 24, 24-dev, 24.1, 24.1-dev, 24.1.1, 24.1.1-dev, 24.1.2, 24.1.2-dev, 24.3, 24.3-dev, 24.3.0, 24.3.0-dev, 24.4, 24.4-dev, 24.4.0, 24.4.0-dev, 24.5, 24.5-dev, 24.5.0, 24.5.0-dev, latest, latest-dev |
...
## Verifying conda Image Signatures
...
cgr.dev/chainguard-private/conda | jq
```

## Downloading conda Image Attestations

The following [attestations](https://slsa.dev/attestation-model) for the conda image can be obtained and verified via cosign:
...
To download an attestation, use the `cosign download attestation` command and provide both the predicate type and the build platform. For example, the following command will obtain the SBOM for the conda image on `linux/amd64`:
...
cosign download attestation \
  ...
  cgr.dev/chainguard/conda | jq -r .payload | base64 -d | jq .predicate
```
...
--platform=linux/amd64 \
--predicate-type=https://spdx.dev/Document \
cgr.dev/chainguard-private/conda | jq -r .payload | base64 -d | jq .predicate
```
...
## Verifying conda Image Attestations
You can use the `cosign verify-attestation` command to check the signatures of the conda image attestations:
...
--certificate-oidc-issuer=https://token.actions.githubusercontent.com \
--certificate-identity=https://github.com/chainguard-images/images-private/.github/workflows/release.yaml@refs/heads/main \
cgr.dev/chainguard-private/conda
```

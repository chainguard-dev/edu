---
title: "Provenance Information for slim-toolkit-debug Images"
type: "article"
unlisted: true
description: "Provenance information for slim-toolkit-debug Chainguard Image"
...
{{< tab title="Provenance" active=true url="/chainguard/chainguard-images/reference/slim-toolkit-debug/provenance_info/" >}}
{{</ tabs >}}
...
### Registry and Tags for slim-toolkit-debug Image
Attestations are provided per image build, so you'll need to specify the correct tag and registry when pulling attestations from an image with `cosign`.

| Registry                     | Tags                   |
|------------------------------|------------------------|
| `cgr.dev/chainguard`         | latest                 |
| `cgr.dev/chainguard-private` | 5, 5.2, 5.2.15, latest |
...
The **slim-toolkit-debug** Chainguard Images are signed using Sigstore, and you can check the included signatures using `cosign`.
...
cgr.dev/chainguard-private/slim-toolkit-debug | jq
```

## Downloading slim-toolkit-debug Image Attestations

The following [attestations](https://slsa.dev/attestation-model) for the slim-toolkit-debug image can be obtained and verified via cosign:
...
To download an attestation, use the `cosign download attestation` command and provide both the predicate type and the build platform. For example, the following command will obtain the SBOM for the slim-toolkit-debug image on `linux/amd64`:
...
--platform=linux/amd64 \
--predicate-type=https://spdx.dev/Document \
cgr.dev/chainguard-private/slim-toolkit-debug | jq -r .payload | base64 -d | jq .predicate
```
...
You can use the `cosign verify-attestation` command to check the signatures of the slim-toolkit-debug image attestations:
...
cosign verify-attestation \
  ...
  --certificate-oidc-issuer=https://token.actions.githubusercontent.com \
  --certificate-identity=https://github.com/chainguard-images/images/.github/workflows/release.yaml@refs/heads/main \
  cgr.dev/chainguard/slim-toolkit-debug
```
...
--certificate-oidc-issuer=https://token.actions.githubusercontent.com \
--certificate-identity=https://github.com/chainguard-images/images-private/.github/workflows/release.yaml@refs/heads/main \
cgr.dev/chainguard-private/slim-toolkit-debug
```

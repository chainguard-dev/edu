...
type: "article"
unlisted: true
description: "Provenance information for wolfi-base Chainguard Image"
...
{{< tab title="Provenance" active=true url="/chainguard/chainguard-images/reference/wolfi-base/provenance_info/" >}}
{{</ tabs >}}
...
### Registry and Tags for wolfi-base Image
Attestations are provided per image build, so you'll need to specify the correct tag and registry when pulling attestations from an image with `cosign`.

| Registry                     | Tags      |
|------------------------------|-----------|
| `cgr.dev/chainguard`         | latest    |
| `cgr.dev/chainguard-private` | 1, latest |
...
## Verifying wolfi-base Image Signatures
...
cosign verify \
  ...
  cgr.dev/chainguard/wolfi-base | jq
```
...
cgr.dev/chainguard-private/wolfi-base | jq
```

## Downloading wolfi-base Image Attestations

The following [attestations](https://slsa.dev/attestation-model) for the wolfi-base image can be obtained and verified via cosign:
...
To download an attestation, use the `cosign download attestation` command and provide both the predicate type and the build platform. For example, the following command will obtain the SBOM for the wolfi-base image on `linux/amd64`:
...
--platform=linux/amd64 \
--predicate-type=https://spdx.dev/Document \
cgr.dev/chainguard-private/wolfi-base | jq -r .payload | base64 -d | jq .predicate
```
...
## Verifying wolfi-base Image Attestations
You can use the `cosign verify-attestation` command to check the signatures of the wolfi-base image attestations:
...
--certificate-oidc-issuer=https://token.actions.githubusercontent.com \
--certificate-identity=https://github.com/chainguard-images/images-private/.github/workflows/release.yaml@refs/heads/main \
cgr.dev/chainguard-private/wolfi-base
```

...
type: "article"
unlisted: true
description: "Provenance information for bash Chainguard Image"
...
{{< tab title="Provenance" active=true url="/chainguard/chainguard-images/reference/bash/provenance_info/" >}}
{{</ tabs >}}
...
### Registry and Tags for bash Image
Attestations are provided per image build, so you'll need to specify the correct tag and registry when pulling attestations from an image with `cosign`.

| Registry                     | Tags                                                                               |
|------------------------------|------------------------------------------------------------------------------------|
| `cgr.dev/chainguard`         | 5.2.15, latest                                                                     |
| `cgr.dev/chainguard-private` | 5, 5-dev, 5.2, 5.2-dev, 5.2.15, 5.2.15-dev, 5.2.21, 5.2.21-dev, latest, latest-dev |
...
## Verifying bash Image Signatures
...
cgr.dev/chainguard-private/bash | jq
```

## Downloading bash Image Attestations

The following [attestations](https://slsa.dev/attestation-model) for the bash image can be obtained and verified via cosign:
...
cosign download attestation \
  ...
  cgr.dev/chainguard/bash | jq -r .payload | base64 -d | jq .predicate
```
...
--platform=linux/amd64 \
--predicate-type=https://spdx.dev/Document \
cgr.dev/chainguard-private/bash | jq -r .payload | base64 -d | jq .predicate
```
...
## Verifying bash Image Attestations
You can use the `cosign verify-attestation` command to check the signatures of the bash image attestations:
...
--certificate-oidc-issuer=https://token.actions.githubusercontent.com \
--certificate-identity=https://github.com/chainguard-images/images-private/.github/workflows/release.yaml@refs/heads/main \
cgr.dev/chainguard-private/bash
```

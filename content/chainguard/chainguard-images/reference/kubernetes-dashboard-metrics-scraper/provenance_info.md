...
### Registry and Tags for kubernetes-dashboard-metrics-scraper Image
...
## Verifying kubernetes-dashboard-metrics-scraper Image Signatures
...
cosign verify \
  ...
  cgr.dev/chainguard/kubernetes-dashboard-metrics-scraper | jq
```
...
cgr.dev/chainguard-private/kubernetes-dashboard-metrics-scraper | jq
```

## Downloading kubernetes-dashboard-metrics-scraper Image Attestations

The following [attestations](https://slsa.dev/attestation-model) for the kubernetes-dashboard-metrics-scraper image can be obtained and verified via cosign:
...
To download an attestation, use the `cosign download attestation` command and provide both the predicate type and the build platform. For example, the following command will obtain the SBOM for the kubernetes-dashboard-metrics-scraper image on `linux/amd64`:
...
cosign download attestation \
  ...
  cgr.dev/chainguard/kubernetes-dashboard-metrics-scraper | jq -r .payload | base64 -d | jq .predicate
```
...
--platform=linux/amd64 \
--predicate-type=https://spdx.dev/Document \
cgr.dev/chainguard-private/kubernetes-dashboard-metrics-scraper | jq -r .payload | base64 -d | jq .predicate
```
...
## Verifying kubernetes-dashboard-metrics-scraper Image Attestations
You can use the `cosign verify-attestation` command to check the signatures of the kubernetes-dashboard-metrics-scraper image attestations:
...
--certificate-oidc-issuer=https://token.actions.githubusercontent.com \
--certificate-identity=https://github.com/chainguard-images/images-private/.github/workflows/release.yaml@refs/heads/main \
cgr.dev/chainguard-private/kubernetes-dashboard-metrics-scraper
```

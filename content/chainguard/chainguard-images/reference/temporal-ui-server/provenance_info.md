...
### Registry and Tags for temporal-ui-server Image
Attestations are provided per image build, so you'll need to specify the correct tag and registry when pulling attestations from an image with `cosign`.

| Registry                     | Tags                                                                                                                                                                                                                                                                                             |
|------------------------------|--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| `cgr.dev/chainguard`         | latest, latest-dev                                                                                                                                                                                                                                                                               |
| `cgr.dev/chainguard-private` | 2, 2-dev, 2.23, 2.23-dev, 2.23.0, 2.23.0-dev, 2.24, 2.24-dev, 2.24.0, 2.24.0-dev, 2.25, 2.25-dev, 2.25.0, 2.25.0-dev, 2.26, 2.26-dev, 2.26.0, 2.26.0-dev, 2.26.1, 2.26.1-dev, 2.26.2, 2.26.2-dev, 2.27, 2.27-dev, 2.27.0, 2.27.0-dev, 2.27.1, 2.27.1-dev, 2.27.2, 2.27.2-dev, latest, latest-dev |
...
cosign verify \
  ...
  cgr.dev/chainguard/temporal-ui-server | jq
```
...
cgr.dev/chainguard-private/temporal-ui-server | jq
```
...
To download an attestation, use the `cosign download attestation` command and provide both the predicate type and the build platform. For example, the following command will obtain the SBOM for the temporal-ui-server image on `linux/amd64`:
...
--platform=linux/amd64 \
--predicate-type=https://spdx.dev/Document \
cgr.dev/chainguard-private/temporal-ui-server | jq -r .payload | base64 -d | jq .predicate
```
...
--certificate-oidc-issuer=https://token.actions.githubusercontent.com \
--certificate-identity=https://github.com/chainguard-images/images-private/.github/workflows/release.yaml@refs/heads/main \
cgr.dev/chainguard-private/temporal-ui-server
```

---
title: "Provenance Information for oidc-discovery-provider Images"
type: "article"
unlisted: true
description: "Provenance information for oidc-discovery-provider Chainguard Image"
...
{{< tab title="Provenance" active=true url="/chainguard/chainguard-images/reference/oidc-discovery-provider/provenance_info/" >}}
{{</ tabs >}}
...
The **oidc-discovery-provider** Chainguard Images are signed using Sigstore, and you can check the included signatures using `cosign`.
...
```shell
cosign verify --certificate-oidc-issuer=https://token.actions.githubusercontent.com --certificate-identity=https://github.com/chainguard-images/images/.github/workflows/release.yaml@refs/heads/main cgr.dev/chainguard/oidc-discovery-provider | jq
```
...
cosign download attestation \
  ...
  cgr.dev/chainguard/oidc-discovery-provider | jq -r .payload | base64 -d | jq
```
...
cosign verify-attestation \
  ...
  cgr.dev/chainguard/oidc-discovery-provider
```

And you should get output that verifies the SBOM signature in cosign's transparency log:

```
Verification for cgr.dev/chainguard/oidc-discovery-provider --

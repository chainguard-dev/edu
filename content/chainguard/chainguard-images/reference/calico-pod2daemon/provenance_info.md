---
title: "Provenance Information for calico-pod2daemon Images"
type: "article"
unlisted: true
description: "Provenance information for calico-pod2daemon Chainguard Image"
...
{{< tab title="Provenance" active=true url="/chainguard/chainguard-images/reference/calico-pod2daemon/provenance_info/" >}}
{{</ tabs >}}
...
The **calico-pod2daemon** Chainguard Images are signed using Sigstore, and you can check the included signatures using `cosign`.
...
```shell
cosign verify --certificate-oidc-issuer=https://token.actions.githubusercontent.com --certificate-identity=https://github.com/chainguard-images/images/.github/workflows/release.yaml@refs/heads/main cgr.dev/chainguard/calico-pod2daemon | jq
```
...
cosign download attestation \
  ...
  cgr.dev/chainguard/calico-pod2daemon | jq -r .payload | base64 -d | jq
```
...
cosign verify-attestation \
  ...
  cgr.dev/chainguard/calico-pod2daemon
```

And you should get output that verifies the SBOM signature in cosign's transparency log:

```
Verification for cgr.dev/chainguard/calico-pod2daemon --

...
type: "article"
unlisted: true
description: "Provenance information for static Chainguard Image"
...
{{< tab title="Provenance" active=true url="/chainguard/chainguard-images/reference/static/provenance_info/" >}}
{{</ tabs >}}
...
### Registry and Tags for static Image
Attestations are provided per image build, so you'll need to specify the correct tag and registry when pulling attestations from an image with `cosign`.

| Registry                     | Tags                 |
|------------------------------|----------------------|
| `cgr.dev/chainguard`         | latest-glibc         |
| `cgr.dev/chainguard-private` | latest, latest-glibc |
...
## Verifying static Image Signatures
...
cgr.dev/chainguard-private/static | jq
```

## Downloading static Image Attestations

The following [attestations](https://slsa.dev/attestation-model) for the static image can be obtained and verified via cosign:
...
cosign download attestation \
  ...
  cgr.dev/chainguard/static | jq -r .payload | base64 -d | jq .predicate
```
...
--platform=linux/amd64 \
--predicate-type=https://spdx.dev/Document \
cgr.dev/chainguard-private/static | jq -r .payload | base64 -d | jq .predicate
```
...
## Verifying static Image Attestations
You can use the `cosign verify-attestation` command to check the signatures of the static image attestations:
...
--certificate-oidc-issuer=https://token.actions.githubusercontent.com \
--certificate-identity=https://github.com/chainguard-images/images-private/.github/workflows/release.yaml@refs/heads/main \
cgr.dev/chainguard-private/static
```

This will pull in the signature for the attestation specified by the `--type` parameter, which in this case is the SPDX attestation. You will receive output that verifies the SBOM attestation signature in cosign's transparency log:

```
Verification for cgr.dev/chainguard/static --

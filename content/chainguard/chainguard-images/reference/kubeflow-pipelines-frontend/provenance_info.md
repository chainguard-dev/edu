...
### Registry and Tags for kubeflow-pipelines-frontend Image
...
cosign verify \
  ...
  cgr.dev/chainguard/kubeflow-pipelines-frontend | jq
```
...
cgr.dev/chainguard-private/kubeflow-pipelines-frontend | jq
```
...
To download an attestation, use the `cosign download attestation` command and provide both the predicate type and the build platform. For example, the following command will obtain the SBOM for the kubeflow-pipelines-frontend image on `linux/amd64`:
...
--platform=linux/amd64 \
--predicate-type=https://spdx.dev/Document \
cgr.dev/chainguard-private/kubeflow-pipelines-frontend | jq -r .payload | base64 -d | jq .predicate
```
...
--certificate-oidc-issuer=https://token.actions.githubusercontent.com \
--certificate-identity=https://github.com/chainguard-images/images-private/.github/workflows/release.yaml@refs/heads/main \
cgr.dev/chainguard-private/kubeflow-pipelines-frontend
```

...
### Registry and Tags for timestamp-authority-cli Image
...
cosign verify \
  ...
  cgr.dev/chainguard/timestamp-authority-cli | jq
```
...
cgr.dev/chainguard-private/timestamp-authority-cli | jq
```
...
To download an attestation, use the `cosign download attestation` command and provide both the predicate type and the build platform. For example, the following command will obtain the SBOM for the timestamp-authority-cli image on `linux/amd64`:
...
--platform=linux/amd64 \
--predicate-type=https://spdx.dev/Document \
cgr.dev/chainguard-private/timestamp-authority-cli | jq -r .payload | base64 -d | jq .predicate
```
...
--certificate-oidc-issuer=https://token.actions.githubusercontent.com \
--certificate-identity=https://github.com/chainguard-images/images-private/.github/workflows/release.yaml@refs/heads/main \
cgr.dev/chainguard-private/timestamp-authority-cli
```

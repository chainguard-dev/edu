---
title: "Provenance Information for busybox-fips Images"
type: "article"
unlisted: true
description: "Provenance information for busybox-fips Chainguard Image"
...
{{< tab title="Provenance" active=true url="/chainguard/chainguard-images/reference/busybox-fips/provenance_info/" >}}
{{</ tabs >}}
...
### Registry and Tags for busybox-fips Image
...
## Verifying busybox-fips Image Signatures
The **busybox-fips** Chainguard Images are signed using Sigstore, and you can check the included signatures using `cosign`.
...
cosign verify \
  ...
  cgr.dev/chainguard/busybox-fips | jq
```
...
cgr.dev/chainguard-private/busybox-fips | jq
```

## Downloading busybox-fips Image Attestations

The following [attestations](https://slsa.dev/attestation-model) for the busybox-fips image can be obtained and verified via cosign:
...
To download an attestation, use the `cosign download attestation` command and provide both the predicate type and the build platform. For example, the following command will obtain the SBOM for the busybox-fips image on `linux/amd64`:
...
cosign download attestation \
  ...
  cgr.dev/chainguard/busybox-fips | jq -r .payload | base64 -d | jq .predicate
```
...
--platform=linux/amd64 \
--predicate-type=https://spdx.dev/Document \
cgr.dev/chainguard-private/busybox-fips | jq -r .payload | base64 -d | jq .predicate
```
...
## Verifying busybox-fips Image Attestations
You can use the `cosign verify-attestation` command to check the signatures of the busybox-fips image attestations:
...
--certificate-oidc-issuer=https://token.actions.githubusercontent.com \
--certificate-identity=https://github.com/chainguard-images/images-private/.github/workflows/release.yaml@refs/heads/main \
cgr.dev/chainguard-private/busybox-fips
```
...
```
Verification for cgr.dev/chainguard/busybox-fips --

---
title: "Provenance Information for kyverno-policy-reporter-ui Images"
type: "article"
unlisted: true
...
{{< tab title="Provenance" active=true url="/chainguard/chainguard-images/reference/kyverno-policy-reporter-ui/provenance_info/" >}}
{{</ tabs >}}
...
### Registry and Tags for kyverno-policy-reporter-ui Image
Attestations are provided per image build, so you'll need to specify the correct tag and registry when pulling attestations from an image with `cosign`.

| Registry                     | Tags                                                                                                           |
|------------------------------|----------------------------------------------------------------------------------------------------------------|
| `cgr.dev/chainguard`         | latest, latest-dev                                                                                             |
| `cgr.dev/chainguard-private` | 1, 1-dev, 1.7, 1.7-dev, 1.7.1, 1.7.1-dev, 1.9, 1.9-dev, 1.9.1, 1.9.1-dev, 1.9.2, 1.9.2-dev, latest, latest-dev |
...
## Verifying kyverno-policy-reporter-ui Image Signatures
The **kyverno-policy-reporter-ui** Chainguard Images are signed using Sigstore, and you can check the included signatures using `cosign`.
...
cgr.dev/chainguard-private/kyverno-policy-reporter-ui | jq
```

## Downloading kyverno-policy-reporter-ui Image Attestations

The following [attestations](https://slsa.dev/attestation-model) for the kyverno-policy-reporter-ui image can be obtained and verified via cosign:
...
cosign download attestation \
  ...
  cgr.dev/chainguard/kyverno-policy-reporter-ui | jq -r .payload | base64 -d | jq .predicate
```
...
--platform=linux/amd64 \
--predicate-type=https://spdx.dev/Document \
cgr.dev/chainguard-private/kyverno-policy-reporter-ui | jq -r .payload | base64 -d | jq .predicate
```
...
## Verifying kyverno-policy-reporter-ui Image Attestations
You can use the `cosign verify-attestation` command to check the signatures of the kyverno-policy-reporter-ui image attestations:
...
--certificate-oidc-issuer=https://token.actions.githubusercontent.com \
--certificate-identity=https://github.com/chainguard-images/images-private/.github/workflows/release.yaml@refs/heads/main \
cgr.dev/chainguard-private/kyverno-policy-reporter-ui
```

This will pull in the signature for the attestation specified by the `--type` parameter, which in this case is the SPDX attestation. You will receive output that verifies the SBOM attestation signature in cosign's transparency log:

```
Verification for cgr.dev/chainguard/kyverno-policy-reporter-ui --

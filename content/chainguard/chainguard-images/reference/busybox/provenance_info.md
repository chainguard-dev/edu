---
title: "Provenance Information for busybox Images"
type: "article"
unlisted: true
...
{{< tab title="Provenance" active=true url="/chainguard/chainguard-images/reference/busybox/provenance_info/" >}}
{{</ tabs >}}
...
### Registry and Tags for busybox Image
Attestations are provided per image build, so you'll need to specify the correct tag and registry when pulling attestations from an image with `cosign`.

| Registry                     | Tags                                                                     |
|------------------------------|--------------------------------------------------------------------------|
| `cgr.dev/chainguard`         | latest-glibc                                                             |
| `cgr.dev/chainguard-private` | 1, 1.36, 1.36.1, glibc-1, glibc-1.36, glibc-1.36.1, latest, latest-glibc |
...
## Verifying busybox Image Signatures
The **busybox** Chainguard Images are signed using Sigstore, and you can check the included signatures using `cosign`.
...
cgr.dev/chainguard-private/busybox | jq
```

## Downloading busybox Image Attestations

The following [attestations](https://slsa.dev/attestation-model) for the busybox image can be obtained and verified via cosign:
...
To download an attestation, use the `cosign download attestation` command and provide both the predicate type and the build platform. For example, the following command will obtain the SBOM for the busybox image on `linux/amd64`:
...
--platform=linux/amd64 \
--predicate-type=https://spdx.dev/Document \
cgr.dev/chainguard-private/busybox | jq -r .payload | base64 -d | jq .predicate
```
...
## Verifying busybox Image Attestations
You can use the `cosign verify-attestation` command to check the signatures of the busybox image attestations:
...
--certificate-oidc-issuer=https://token.actions.githubusercontent.com \
--certificate-identity=https://github.com/chainguard-images/images-private/.github/workflows/release.yaml@refs/heads/main \
cgr.dev/chainguard-private/busybox
```

This will pull in the signature for the attestation specified by the `--type` parameter, which in this case is the SPDX attestation. You will receive output that verifies the SBOM attestation signature in cosign's transparency log:

```
Verification for cgr.dev/chainguard/busybox --

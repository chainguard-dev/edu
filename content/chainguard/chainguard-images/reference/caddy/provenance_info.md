...
### Registry and Tags for caddy Image
Attestations are provided per image build, so you'll need to specify the correct tag and registry when pulling attestations from an image with `cosign`.

| Registry                     | Tags                                                                                                           |
|------------------------------|----------------------------------------------------------------------------------------------------------------|
| `cgr.dev/chainguard`         | latest, latest-dev                                                                                             |
| `cgr.dev/chainguard-private` | 2, 2-dev, 2.7, 2.7-dev, 2.7.6, 2.7.6-dev, 2.8, 2.8-dev, 2.8.1, 2.8.1-dev, 2.8.4, 2.8.4-dev, latest, latest-dev |
...
## Verifying caddy Image Signatures
...
cgr.dev/chainguard-private/caddy | jq
```

## Downloading caddy Image Attestations

The following [attestations](https://slsa.dev/attestation-model) for the caddy image can be obtained and verified via cosign:
...
To download an attestation, use the `cosign download attestation` command and provide both the predicate type and the build platform. For example, the following command will obtain the SBOM for the caddy image on `linux/amd64`:
...
cosign download attestation \
  ...
  cgr.dev/chainguard/caddy | jq -r .payload | base64 -d | jq .predicate
```
...
--platform=linux/amd64 \
--predicate-type=https://spdx.dev/Document \
cgr.dev/chainguard-private/caddy | jq -r .payload | base64 -d | jq .predicate
```
...
## Verifying caddy Image Attestations
You can use the `cosign verify-attestation` command to check the signatures of the caddy image attestations:
...
--certificate-oidc-issuer=https://token.actions.githubusercontent.com \
--certificate-identity=https://github.com/chainguard-images/images-private/.github/workflows/release.yaml@refs/heads/main \
cgr.dev/chainguard-private/caddy
```

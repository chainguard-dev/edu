...
Due to the complexity of cross-referencing image components against databases, scanners are not infallible. Sometimes vulnerabilities are misidentified — or even missed entirely. There are numerous ways in which this can occur throughout the scanning process from start to finish.
...
To address the inconsistencies caused by software vendors using proprietary version naming schemes, adopting the [purl specification](https://github.com/package-url/purl-spec) can help. A purl, or package URL, aims to standardize versioning by outlining a convention that incorporates pertinent package information in every identifier. Using purls can [reduce the number of false positives which surface](https://www.chainguard.dev/unchained/a-purl-of-wisdom-on-sboms-and-vulnerabilities) by making it easier for scanners to align version information between data sources.

Utilizing the [Vulnerability Exploitability eXchange](), or VEX, can provide insight on the status of software vulnerabilities. <VEX - discuss how VEX and OpenVEX can help developers understand what is surfacing in their scans - if there is need for action, if a fix is planned, etc. helps streamline taking the next step in the course of action>
...
A primary source of large vulnerability counts reported in scanners is the dead weight caused by unnecessary dependencies. Many popular container images contain hundreds of packages, each with their own potential to introduce vulnerabilities, true and false positives. Having so much to sift through makes vulnerability management a daunting and cyclical task, eating through development time.

[Chainguard Images](https://www.chainguard.dev/chainguard-images), built on the Wolfi un-distro, can help you reduce your CVE count dramatically by keeping things minimal. By bundling only what is necessary to run the image, Chainguard Images are hardened and lightweight in comparison to their popular counterparts. To learn more about how Chainguard Images can help you achieve low (or no) CVEs in your containers, check out our [Images documentation](/chainguard/chainguard-images/reference/)

## Learn More

Vulnerabilities as a whole can be a major pain point for developers, with false positives and negatives complicating the remediation process further. Triaging and addressing true positive vulnerabilities — with false positives mixed in, and false negatives left out — can eat through development resources quickly. 

In this article, you learned how false results from your vulnerability scanners can occur, and how they can impact your development workflow. Additionally, you explored various ways you can improve the accuracy of your scanner through the application of tools such as SBOMs or VEX, and by choosing container images which reduce excess components.

To learn more, you can check out our [collection of articles on SBOMs and VEX](/open-source/sbom/), read about [selecting a base image](/software-security/selecting-a-base-image/), or. 

Learn how Chainguard Images can help you [reach zero CVEs in your containers](https://www.chainguard.dev/chainguard-images).

To learn about choosing the right scanner tool for your application, check out [this comparison of SCA and SAST scanning tools](https://github.blog/2022-09-09-sca-vs-sast-what-are-they-and-which-one-is-right-for-you/
).

...
```shell
chainctl policy-gate describe --policy=$POLICY
```
...
```shell
chainctl policy-gate enable --policy=no-eol --mode=DRY_RUN
```

Some policies accept parameters. Use `--param=KEY=VALUE` to supply them:

```shell
chainctl policy-gate enable --policy=cooldown --mode=DRY_RUN --param=days=7
```

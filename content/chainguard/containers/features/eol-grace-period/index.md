...
- /chainguard/containers/features/eol-gp-overview/
---
...
It's recommended that when a software version reaches the EOL phase, users should migrate their projects to a later version, as [EOL software is known to accumulate vulnerabilities](/chainguard/containers/staying-secure/updating-containers/how-eol-software-accumulates-cves/). However, there are cases where an organization may want to continue using a container image after it has reached end-of-life. This could be because an image reaches EOL before the organization's release schedule, or perhaps later image versions have one or more issues that prevent the organization from upgrading.
...
2. Has [multiple release tracks](/chainguard/containers/about/versions/#how-versions-are-maintained)

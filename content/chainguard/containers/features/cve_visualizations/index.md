...
- /chainguard/containers/working-with-images/cve_visualizations/
...
The CVE data used in these reports is from the [Grype vulnerability scanner](/chainguard/containers/staying-secure/working-with-scanners/grype-tutorial/). Vulnerability data is constantly evolving, so we scan container images each day and store the results. The results shown are the vulnerabilities found on the day in question; scanning the container images again with a newer database will show different results.

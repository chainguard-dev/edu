...
| Feature                             | Description |
|:------------------------------------| :---- |
| **FIPS 140-3 validated crypto**     | [NIST](https://www.nist.gov/) CMVP-validated software modules and [SP 800-90B](https://csrc.nist.gov/pubs/sp/800/90/b/final) compliant entropy, with runtime guardrails blocking non-FIPS crypto. |
...
| **Compliance evidence & reporting** | FIPS certificates, OpenSSL docs, SCAP scan results, and POA\&M-ready artifacts. |
| **CVE remediation SLA**             | 7 days for critical CVEs, 14 days for high, medium, and low. |

Chainguard FIPS 140-3 validated and hardened VM images serve as ready-to-use replacements for standard operating systems across AWS, Azure, and GCP, allowing organizations to maintain existing infrastructure and workflows while achieving immediate compliance. Read on this doc to learn more about Chainguard VMs compliance features and how they can help reduce engineering toil from your organization.
...
* **Runtime guardrails**
    * Guardrails prevent the use of non-FIPS-approved cryptography at runtime (e.g., enforcing FIPS-approved ciphers and modes only).
...
Chainguard VMs provide variants hardened to DISA Security Technical Implementation Guide (STIG) requirements used across U.S. federal and defense environments.
...
For organizations standardizing on CIS controls, Chainguard offers images hardened to **CIS Level 1** benchmarks.

* Images are available in **CIS L1 hardened variants** for Chainguard VMs.
* Chainguard VMs use a **hybrid baseline combining CIS L1 benchmarks with STIG requirements and industry-recognized secure defaults** to provide defense-in-depth hardening.

This allows security and GRC (Governance, Risk, and Compliance) teams to map infrastructure posture to both internal CIS-based policies and external STIG-based requirements without maintaining parallel baselines.
...
Chainguard VMs are designed to simplify generation of compliance artifacts often required in audits, ATO processes, and customer security reviews.
...
* POA\&M-ready information (Plans of Action and Milestones) to support risk tracking and remediation planning.
...
* Images are updated regularly and made available to customers within these SLA windows.
* The minimal, hardened footprint reduces the volume of installed software and therefore, the attack surface and CVE count that teams must manage.

This lifecycle management shifts ongoing compliance from a perpetual engineering project into a managed image-consumption model.

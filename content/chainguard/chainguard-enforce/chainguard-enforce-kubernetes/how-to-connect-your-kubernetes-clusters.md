...
type: "article"
lead: ""
description: "Connecting our containerized workloads to Chainguard Enforce for Kubernetes"
...
to Enforce. Clusters using the Enforce Agent are only required to have [Service
...
Next create a pod specification that uses a projected volume of the
...
If Service Account Issuer Discover is enabled you should see JSON output similar to 
...
If it is not enabled you should see an error message similar to
...
To connect a public cluster to Enforce, run
...
association between the GCP project hosting the cluster and the Enforce IAM
...
association is correctly configured run

```sh
chainctl iam group check-gcp {group-id}
```
...
```sh
chainctl cluster install --group={group id} --managed=gke --cluster={cluster-name}
```

Once connected, you should see you cluster listed
...
```sh
chainctl iam group check-aws {group-id}
```

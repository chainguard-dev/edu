...
`limited_owner` is a special role designed for [Catalog Starter](/chainguard/containers/about/catalog-starter/) users, combining the capabilities of the `viewer` role with pull token creation permissions. It does not allow users to invite others to the organization or access paid features like Custom Assembly.

...
```
chainctl auth pull-token create [--save=true|false] [--name=NAME] [--description=DESC] [--ttl=NUM_HOURS_ACTIVE] [--parent=PARENT] [--repository={oci|apk|java|javascript|java_athena|javascript_athena|dotnet|dotnet_athena|python|python_athena}] [flags]
```
...
```
      ...
      --repository string    The repository type to create a pull token for. Must be one of: oci, apk, java, javascript, java_athena, javascript_athena, dotnet, dotnet_athena, python, python_athena. (default "oci")

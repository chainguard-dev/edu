...
```
chainctl auth pull-token create [--save=true|false] [--name=NAME] [--description=DESC] [--ttl=NUM_HOURS_ACTIVE] [--parent=PARENT] [--repository={oci|apk|java_athena|python_athena|dotnet_athena|go_athena|python|javascript|javascript_athena|dotnet|go|java}] [flags]
```
...
```
      ...
      --repository string    The repository type to create a pull token for. Must be one of: oci, apk, java_athena, python_athena, dotnet_athena, go_athena, python, javascript, javascript_athena, dotnet, go, java. (default "oci")

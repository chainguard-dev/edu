...
```
chainctl auth pull-token create [--save=true|false] [--name=NAME] [--description=DESC] [--ttl=NUM_HOURS_ACTIVE] [--parent=PARENT] [--repository={oci|apk|java|javascript|java_athena|python_athena|javascript_athena|go_athena|python|dotnet|dotnet_athena|go}] [flags]
```
...
```
      ...
      --repository string    The repository type to create a pull token for. Must be one of: oci, apk, java, javascript, java_athena, python_athena, javascript_athena, go_athena, python, dotnet, dotnet_athena, go. (default "oci")

...
```
chainctl auth pull-token create [--save=true|false] [--name=NAME] [--description=DESC] [--ttl=NUM_HOURS_ACTIVE] [--parent=PARENT] [--repository={oci|apk|python|javascript|java_athena|python_athena|dotnet|dotnet_athena|java|javascript_athena|go|go_athena}] [flags]
```
...
```
      ...
      --repository string    The repository type to create a pull token for. Must be one of: oci, apk, python, javascript, java_athena, python_athena, dotnet, dotnet_athena, java, javascript_athena, go, go_athena. (default "oci")

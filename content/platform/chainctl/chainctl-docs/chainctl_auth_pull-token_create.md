...
```
chainctl auth pull-token create [--save=true|false] [--name=NAME] [--description=DESC] [--ttl=NUM_HOURS_ACTIVE] [--parent=PARENT] [--repository={oci|apk|go_athena|java|javascript|dotnet|dotnet_athena|go|python|java_athena|python_athena|javascript_athena}] [flags]
```
...
```
      ...
      --repository string    The repository type to create a pull token for. Must be one of: oci, apk, go_athena, java, javascript, dotnet, dotnet_athena, go, python, java_athena, python_athena, javascript_athena. (default "oci")

...
```
chainctl auth pull-token list [--parent=PARENT] [--expired=true|false] [--repository={oci|apk|java|python|javascript|java_athena|python_athena|javascript_athena}] [flags]
```
...
```
      ...
      --repository string   The repository type to list pull tokens for. Must be one of: oci, apk, java, python, javascript, java_athena, python_athena, javascript_athena
```

...
```
chainctl auth pull-token list [--parent=PARENT] [--expired=true|false] [--repository={oci|apk|javascript|java_athena|python_athena|javascript_athena|java|python}] [flags]
```
...
```
      ...
      --repository string   The repository type to list pull tokens for. Must be one of: oci, apk, javascript, java_athena, python_athena, javascript_athena, java, python
```

...
You'll also need to download the Chainguard Raspberry Pi Docker Image using the form in [this link](http://images.chainguard.dev/rpi).
...
This will bootstrap a new Minecraft Java server using default settings configured via environment variables on the `docker-compose.yaml` file. You can connect from any compatible client on your local network.
...
Next, run the command to build the `wolfi-llama` container image. This will compile Llama.cpp from source, which may take several minutes to complete.
